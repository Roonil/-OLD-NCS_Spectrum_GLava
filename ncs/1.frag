...
float fbm3(vec4 p,float disp,float flow) {
  
    float perlinVal = cnoise(vec4(p.xyz+flow*time,p.w),vec4(0)), maxDisp =(audioDisperse3+ audioFractal1+audioFractal2+audioFractal3)*disp;
...
void main() {
    ...
    float t = .015 * time, K = 2.5, S = 2.+3.5*(audioDisperse1+audioDisperse2);

S=1.4;

   // fragment.xyz += vec3(fbm3(vec3(S * fragment.xy, t * 2.)), fbm3(vec3(S * fragment.xz, t * 3.)), fbm3(vec3(S * fragment.yz, t))) / S;
    float evolution = time*.01, displaceX = .8, displaceY = .8, displaceZ = min(.8+2.1*audioDisperse1+1.8*audioDisperse2,3.), flowX = 0.01, flowY = .03, flowZ=0;
    // displaceZ = .8+2.5*audioDisperse1
    
    fragment.xyz += vec3(fbm3(vec4(S * vec4(fragment.xyz,evolution)),displaceX,flowX), fbm3(vec4(S * vec4(fragment.yzx,evolution)),displaceY,flowY), fbm3(vec4(S * vec4(fragment.zxy,evolution)),displaceZ,flowZ)) / S; //1.25
  ...
  // fragment.xyz = radius * normalize(fragment.xyz);
    ...
    fragment.xy = .5 + fragment.xy / 7.7 * vec2(screen.y/screen.x, 1);
