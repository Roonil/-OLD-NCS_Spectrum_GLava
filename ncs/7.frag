...
vec3 getAvgColorAround(vec2 uv){
  ...
  //return col;
    for(int i = -5; i < 5; i++) for(int j = -5; j < 5; j++) 
            col += texture(prev, vec2(uv.x + float(i) / 60, uv.y + float(j) / 60.)).xyz;
...
//o.r*=length(o.gb);
    vec3 col = vec3(0.1529, 0.0039, 0.3922)  ;   //vec3(0.0, 0.2667, 1.0) = Blue vec3(0.0, 0.4, 1.0) = Cyan  vec3(1.0, 0.451, 0.0) = Orange vec3(0.5686, 0.0, 0.0) = Red vec3(0.4353, 0.0, 1.0) = Magenta vec3(0.2353, 0.0, 0.4588) = Deep Purple vec3(0.0, 0.1569, 0.4471) = Deep Blue vec3(0.3647, 0.0, 0.5333) = Matte Purple vec3(0.1216, 0.0, 0.4549) = Royal Blue vec3(0.2118, 0.0, 0.4549) = Matte Violet vec3(0.0, 0.1216, 0.4549) = Demon Blue vec3(0.4549, 0.0, 0.098) = Crimson vec3(0.3686, 0.3686, 0.3686) = Silver
